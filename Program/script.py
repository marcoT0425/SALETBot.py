...
def calculate_solve_analytics(candidate, is_hard, current_pool, current_turn):
    ...
    five_plus = sum(stats[4:])
    return win_p, exp, max_turns, missed, stats, five_plus
    ...
    while True:
        ...
        if turn_count > 1:
            ...
            for i, (w, _) in enumerate(base_recs[:actual_to_analyze], 1):
                wp, exp, worst, miss, st, fpc = calculate_solve_analytics(w, hard_mode, propers_remaining, turn_count)
                enriched.append((w, wp, exp, worst, miss, st, fpc))
                sys.stdout.write(f"\rProgress: {int((i / actual_to_analyze) * 100)}%")
                sys.stdout.flush()

            # TABLE HEADER ALIGNMENT
            print("\n\n" + f"{'WORD':<10} | {'WIN %':<7} | {'EXP':<7} | {'WORST':<5} | {'5+ CT':<5} | {'STATS [1,2,3,4,5,6,X]'}")
            print("-" * 110)

            # HIERARCHY: #1 Rate (desc), #2 Avg (asc), #3 Worst (asc), #4 5+ (asc), #5 A-Z (asc)
            enriched.sort(key=lambda x: (-x[1], x[2], x[3], x[6], x[0]))

            for rw, wp, rg, rwst, rm, rst, fpc in enriched[:15]:
