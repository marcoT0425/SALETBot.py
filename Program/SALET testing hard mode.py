...
def get_best_sim_move(pool_tuple, is_hard, prev_guess, last_pattern):
    ...
    # Strictly filter for Hard Mode candidates
    ...
    for secret in current_pool:
        ...
        while s_t <= 6:
            ...
            if s_t > 6:
                ...
                stats[6] += 1
                break
            s_g = get_best_sim_move(tuple(s_p), True, s_g, p)
    return ((pool_size - len(missed)) / pool_size) * 100, total_turns / pool_size, max_turns, missed, stats


# --- 4. ULTIMATE BATCH PLAYTHROUGH (NO RESUME) ---
def run_benchmark():
    load_data()
    LIMIT = 100
    starter = input("Enter starting word: ").lower().strip()
    hard_mode = True  # Always on
    ...
    print(f"\nStarting fresh 2315 Playthrough for '{starter.upper()}' (Hard Mode)...")
    ...
    for i in range(len(proper_word)):
        ...
        while turn <= 6:
            ...
            state_key = (curr_pool, curr_word, p)
            if state_key in decision_cache:
                curr_word = decision_cache[state_key]
            else:
                cands = [c for c in full_dictionary if is_hard_mode_valid(c, curr_word, p)]
                base_recs = []
                for c in cands:
                    pg = {}
                    for s in curr_pool:
                ...
                for w_cand, _ in base_recs[:LIMIT]:
                    wp, exp, worst, miss, st = calculate_solve_analytics(w_cand, True, list(curr_pool), turn)
